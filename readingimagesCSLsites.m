...
%before running, make sure the function 'FindinMetadata' is one that can
%return both nums or strings, updated from Julia's
...
[ImageMat, Metadata] = extractingseries(Image, 2);
        ...
        if isempty(CheckIfMatFile) %with this if we will check if there is a .mat file. If there isn't, the matrix will be read and the mini app will be generated
           ...
           ROIdim = [100,100,90,40];
                 ...
                 if (Metadata(s).SeriesBits == 8)
                   ...
                   app = bandsintensitiesCSLsites(ImageMat(s).Image, ROIdim);
                   while isvalid(app); pause(0.1); end


                    IntensityROI = ExportIntensityROI;  
                    IntensityROI2 = ExportIntensityROI2;
                    ...
                    save(append(LifLocString, ' series ', Metadata(s).SeriesName, ' ROIMatrixextraband.mat'), 'IntensityROI2');
