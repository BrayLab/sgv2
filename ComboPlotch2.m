...
nameofmolofinterest = 'mCherry::CSL';
nameofloctag = "loctag";




listConditions = {'ON0', 'OFF'};
...
listColor2 = cat(1, suhgreenon, suhgreenoff);
...
Yaxislastvalue = NewPixelSize * size(ImageMatSimp2, 1);
Xaxislastvalue = NewPixelSize * size(ImageMatSimp2, 2);
...
topcurve = (mean(ChosenNorm2, [1 3], 'omitnan') + SEMmolofinterest2).';
lowercurve = (mean(ChosenNorm2, [1 3], 'omitnan') - SEMmolofinterest2).';
...
pline1 = plot(spatialscale, mean(ChosenNorm2, [1 3], 'omitnan'), '-', 'DisplayName', nameofmolofinterest, 'color', listColor(a,:));
...
% 
plotBoxplotMATLABlike(AllMatrix2, listConditions, listConditions, 0.5 , 0.25, '10 middle pixels', 14, 14, MyColorMap, 0.3, 5, 18, [LOWERylim,  UPPERylim])
...
hold off
plotBoxplotMATLABlike(AllMatrix2, listConditions, listConditions, 0.5, 0.25, '', 14, 20, MyColorMap, 0.3, 5, 35, [LOWERylim, UPPERylim])
...
orient(gcf,'landscape');
saveas(gcf, append(path, FileSep , 'Combo plot ch2', '.pdf'));

[p, h] = ranksum(AllMatrix2(:, 2),AllMatrix2(:, 1))
