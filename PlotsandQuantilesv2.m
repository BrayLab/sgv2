...
FileSep = filesep;
f = filesep;
% Define a default molecule name
defaultMoleculeName = 'DefaultMoleculeName';

% Load the last used molecule name if it exists
if exist('lastMoleculeName.mat', 'file')
    load('lastMoleculeName.mat', 'lastMoleculeName');
else
    lastMoleculeName = defaultMoleculeName;
end

% Ask the user for the name of the molecule, using the last used name as a default
userInput = input(['Enter the name of the molecule (press Enter to use the default "' lastMoleculeName '"): '], 's');
if ~isempty(userInput)
    % If the user input is not empty, update the lastMoleculeName
    lastMoleculeName = userInput;
    % Save the lastMoleculeName to a MAT file for future use
    save('lastMoleculeName.mat', 'lastMoleculeName');
else
    % If the user presses Enter without typing a new name, use the default value
    lastMoleculeName = defaultMoleculeName;
end

% Use lastMoleculeName in your script
nameofmolofinterest = lastMoleculeName;
...
q1 = quantile(MiddleFoldChange, 0.25);  % 25th percentile (1st quartile)
q2 = quantile(MiddleFoldChange, 0.50);  % 50th percentile (2nd quartile, also known as the median)
q3 = quantile(MiddleFoldChange, 0.75);  % 75th percentile (3rd quartile)
Quartile1 = [];
Quartile2 = [];
Quartile3 = [];
Quartile4 = [];
% warning
for u = 1:size(MiddleFoldChange, 1)
    if discretize(MiddleFoldChange(u), [0, quantile(MiddleFoldChange, 0.25)])==1
        Quartile1 = cat(3, Quartile1, onethirdImageMat1(:,:, u));
    elseif discretize(MiddleFoldChange(u), [quantile(MiddleFoldChange, 0.25), quantile(MiddleFoldChange, 0.5)])==1
        Quartile2 = cat(3, Quartile2, onethirdImageMat1(:,:, u));
    elseif discretize(MiddleFoldChange(u), [quantile(MiddleFoldChange, 0.5), quantile(MiddleFoldChange, 0.75)])==1
        Quartile3 = cat(3, Quartile3, onethirdImageMat1(:,:, u));
    else
        Quartile4 = cat(3, Quartile4, onethirdImageMat1(:,:, u));
    end
end
%Plotting quartiles
...
% LOWERylim = 0;
% UPPERylim = 6;
QuartileFigure = figure('Position', [100, 100, 400, 1000]);
tiledlayout(4, 1)
nexttile
imagesc(mean(Quartile1, 3));
title ('Q1');
ylabel('Distance (um)');
    xlabel('Distance (um)');
    colorbar;
...
saveas(gcf, append(path, FileSep, 'Quartiles of', nameofmolofinterest, ' CROPPEDwith1-3', listConditionsNAME{a}, 'treatment.pdf'));
